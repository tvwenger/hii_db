...
def compute_distances(
    ...
    rows = []
    for i in range(len(data) // 500 + 1):
        mydata = data[i * 500 : (i + 1) * 500]
...
def main(
    db, num_samples=5000, rotcurve="reid19_rotcurve", tablename="Distances_Reid2019"
):
    """
    Compute the Monte Carlo Reid+2019 kinematic distances.
    Reset database table and populate new table.
    ...
    compute_distances(
        data, db, num_samples=num_samples, rotcurve=rotcurve, tablename=tablename
    )
